...
from dolfinx.fem import DofMapRestriction, functionspace, locate_dofs_topological
...
def TaylorHoodFunctionSpace(mesh, family_degree):
    ...
    V_element = basix.ufl.element(
        family, mesh.basix_cell(), degree + 1, shape=(mesh.geometry.dim, ), dtype=default_real_type)
    Q_element = basix.ufl.element(family, mesh.basix_cell(), degree, dtype=default_real_type)
...
def get_function_spaces():
    return (
        ...
        lambda mesh: functionspace(mesh, ("Lagrange", 1, (mesh.geometry.dim, ))),
        lambda mesh: functionspace(mesh, ("Lagrange", 2, (mesh.geometry.dim, ))),
        lambda mesh: functionspace(mesh, ("Lagrange", 1, (mesh.geometry.dim, mesh.geometry.dim))),
        lambda mesh: functionspace(mesh, ("Lagrange", 2, (mesh.geometry.dim, mesh.geometry.dim))),
