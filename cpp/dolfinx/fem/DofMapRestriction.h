...
public:
  ...
  DofMapRestriction(std::shared_ptr<const DofMap> dofmap,
...
private:
  ...
  void _map_owned_dofs(std::shared_ptr<const DofMap> dofmap,
                       const std::vector<std::int32_t>& restriction);

  /// Helper function for constructor: map ghost unrestricted dofs into restricted ones
  void _map_ghost_dofs(std::shared_ptr<const DofMap> dofmap,
                       const std::vector<std::int32_t>& restriction);

  /// Helper function for constructor: compute cell dofs arrays in _list
  void _compute_cell_dofs(std::shared_ptr<const DofMap> dofmap);
  ...
  std::shared_ptr<graph::AdjacencyList<std::int32_t>> _list;
};
} // namespace fem
} // namespace dolfinx
